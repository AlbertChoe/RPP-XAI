...
def main():
    ...
    indices_map["True_HighRisk"] = get_idx(
        (y_true == 1) & (preds == 1), indices_map.values()
    )
    indices_map["True_LowRisk"] = get_idx(
        (y_true == 0) & (preds == 0), indices_map.values()
    )
    indices_map["False_HighRisk"] = get_idx(
        (y_true == 0) & (preds == 1), indices_map.values()
    )
    indices_map["False_LowRisk"] = get_idx(
        (y_true == 1) & (preds == 0), indices_map.values()
    )
    ...
    if current_count < required_count:
        ...
        print(
            f"   Found {current_count} specific cases. Adding {needed} random cases..."
        )
        ...
        random_selection = random.sample(
            available_indices, min(needed, len(available_indices))
        )
